...
char strcmp(char * arr1, char * arr2, int length);
...
int main() 
{
    int succ = 0;
    char *argv[2];
    // inisialisasi variabel global PCB
    initializeProcStructures();
    makeInterrupt21();
    makeTimerInterrupt();
    
    interrupt(0x10, 0x200, 0, 0, 0);
    // Set argumen default pada sektor
...
void findDir(char * parent, char * current, char * filename, int * idx, int * result) 
{
    ...
    while ((!found) && (i < MAX_DIRS)) {
        char buff[4];
        int k = 0;
        if ((dir[i * ENTRY_LENGTH] == *parent) && (strcmp(name, dir + (i * ENTRY_LENGTH) + 1, MAX_FILENAME))){
            found = TRUE;
        } else {i++;}
    }
    if (found) {
...
{
	// (Dipanggil jika terjadi timer interrupt)
    struct PCB *currPCB;
    struct PCB *nextPCB;
...
void pauseProcess (int segment, int *result) 
{
	//Membuat state dari suatu program menjadi PAUSED
    struct PCB *pcb;
    int res;
...
void resumeProcess (int segment, int *result) 
{
	// Menjalankan program yang sebelumnya paused
    struct PCB *pcb;
    int res;
...
char strcmp(char *s1, char *s2, int len) 
{
    ...
    int equal = TRUE;
    while ((equal) && (i < len)) {
		// Cek setiap karakter sama atau tidak
        equal = (s1[i] == s2[i]);
        if (equal) {
            if (s1[i] == '\0') {
                i = len;
