...
void printString(char *string){
	...
	while(string[i] != '\0'){
		ch = string[i];
		if ((ch) == '\n') {
			//Selalu tambahkan return setelah newline
...
void readString(char *string, int disableProcessControls) 
{
    ...
    while (TRUE) {
        ...
        if ((c == 3) && (!disableProcessControls)) {
            ...
            //Melakukan print ^C
            printString("^C\n");
            printString("Program diterminasi\n");
        ...
        } else if (c == '\r') { //return atau newline
            interrupt(0x10, 0xE00 + '\r', 0, 0, 0);
...
void findFile(char * parent, char * current, char * filename, int * idx, int * result) 
{
    ...
    int i = 0;
    if (filename[*idx] == '/'){*idx++;}
    //Cek apakah input langsung menunjuk file atau terdapat pada direktori tertentu
    //Misal abc/def.txt, copy 'abc'
    while ((filename[*idx + i] != '\0') && (filename[*idx + i] != '/')) {
        name[i] = filename[*idx + i];
        i++;
    }
    
    //Cek apakah file atau bukan
    file = (filename[*idx + i] == '\0');
    name[i] = '\0';
    j = i;
    if (file) {
		//read file dari sektor
        readSector(dir, FILES_SECTOR);
        count = MAX_FILES;
    } else {
		//read direktori dari sektor
...
void readFile(char *buffer, char *path, int *result, char parentIndex) 
{
    ...
    if (*result == SUCCESS) {
        ...
        while ((copying) && (i < MAX_SECTORS)) {
            ...
            if (sector == INSUFFICIENT_SECTORS) {
				//Sektor kurang
				copying = FALSE;
			} else{
				readSector(buffer + i * SECTOR_SIZE, sector);
			}
            i++;
        }
        *result = current;
...
void deleteFileIndex(char current) 
{
    ...
    char files[SECTOR_SIZE];    
    int j = 0;
    //Baca sector terlebih dahulu
    readSector(files, FILES_SECTOR);
    readSector(sectors, SECTORS_SECTOR);
    readSector(map, MAP_SECTOR);
    
    files[current * ENTRY_LENGTH] = 0x00;
...
void makeDirectory(char *path, int *result, char parentIndex) 
{
    ...
    if (*result == SUCCESS) {
        *result = ALREADY_EXISTS;
        ...
        char filename[MAX_FILENAME + 1];
        int j = 0;
        // Copy direktory pertama yang terbaca
...
void resumeProcess (int segment, int *result) 
{
    ...
    *result = res;
    // Perlu tambahkan sleep jika sukses
}
