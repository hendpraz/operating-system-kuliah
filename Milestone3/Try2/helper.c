...
//Fungsi-fungsi pembantu program utilitas
...
{
    // Mencari index dari directory berdasarkan parent index
    ...
    // Copy bagian awal nama folder 'abc/def', ambil 'abc'
    i = 0;
    ...
    if (filename[i] == '/')
    {
        // cara direktory dengan path nama (directory name)
...
void findFile(char *name, char *result, char parentIndex, int *success)
{
    ...
    // parentIndex dimana index disimpan di result dan success
    char files[SECTOR_SIZE];
    char nama[20];
    int i = 0;
    int j, pathLen;
    ...
    // jika pathLen == 0, berarti masukan string kosong
    if (pathLen == 0)
    {
        *success = -1;
        return;
